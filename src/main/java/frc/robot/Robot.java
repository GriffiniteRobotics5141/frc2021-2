...
public class Robot extends TimedRobot {
  ...
  /*
   * private final I2C.Port cPort = I2C.Port.kOnboard; private final ColorSensorV3
   * cSensor = new ColorSensorV3(cPort); private final ColorMatch m_colorMatcher =
   * new ColorMatch(); private final Color BlueTarget =
   * ColorMatch.makeColor(0.143, 0.427, 0.429); private final Color GreenTarget =
   * ColorMatch.makeColor(0.197, 0.561, 0.240); private final Color RedTarget =
   * ColorMatch.makeColor(0.561, 0.232, 0.114); private final Color YellowTarget =
  ...
  public void robotPeriodic() {
    /*
     * int proximity = cSensor.getProximity(); Color detectedColor =
     * cSensor.getColor(); ColorMatchResult match =
     * m_colorMatcher.matchClosestColor(detectedColor); gameData =
     * DriverStation.getInstance().getGameSpecificMessage();
     * 
     * if (match.color == BlueTarget) { colorString = "B"; } else if (match.color ==
     * RedTarget) { colorString = "R"; } else if (match.color == GreenTarget) {
     * colorString = "G"; } else if (match.color == YellowTarget) { colorString =
     * "Y"; } else { colorString = "Unknown"; }
     * 
     * double IR = cSensor.getIR(); SmartDashboard.putNumber("Red",
     * detectedColor.red); SmartDashboard.putNumber("Green", detectedColor.green);
     * SmartDashboard.putNumber("Blue", detectedColor.blue);
     * SmartDashboard.putNumber("IR", IR); SmartDashboard.putNumber("Proximity",
     * proximity); SmartDashboard.putNumber("Confidence", match.confidence);
    ...
    ratioY = (Math.abs(disXnum - setDistance) < 25) ? (disXnum - setDistance) / 25
        : 1 * Math.signum(disXnum - setDistance); // Based of distance of target from dsXnum 108, 35
    ...
    double sineNavX = Math.signum(ratioNavX) * ((maxCorrectNavX - minCorrectNavX) / 2)
        * Math.sin(Math.PI * (ratioNavX - .5)) + Math.signum(ratioNavX) * ((maxCorrectNavX + minCorrectNavX) / 2);
  ...
  public void autonomousPeriodic() {
     ...
     * -.5); // Put right auto code here break; case kCenter: if
     * (autonamousTimer.get() == 0) { autonamousTimer.start(); } if
     * (autonamousTimer.get() <= 1) { driveTrain.tankDrive(-.5, -.5);
     * shooter.set(ControlMode.PercentOutput, 0); //0.8 } else if (v != 1) {
     * driveTrain.tankDrive(.45, -.45); } else if (v == 1) { if (autoPeriod.get() ==
     * 0) { autoPeriod.start(); } doAutoPilotNow = true; if (autoPeriod.get() > 2 &&
     * autoPeriod.get() <= 4) { doAutoPilotNow = false; conveyor.set(.7);
     * intake.set(.5); } }
     * 
  ...
  public void teleopPeriodic() {
    ...
    /*
     * if (gameData.length() > 0) { switch (gameData.charAt(0)) { case 'B':
     * gameSadFace = "R"; break; case 'G': gameSadFace = "Y"; break; case 'R':
     * gameSadFace = "B"; break; case 'Y': gameSadFace = "G"; break; default:
     * gameSadFace = "N/A"; break; } } SmartDashboard.putNumber("seenColor",
     * seenColor);
     */
    ...
    if (manualMode) {
      ...
      if (gamePad0.getRawButton(6)) {// shooter Righth bumper
        // shooter.set(ControlMode.PercentOutput, 0.8/* veloFwoosh*velocityToMotorRatio
        // */);// shooter value depending on target distance x and y
      } else {
        // shooter.set(ControlMode.PercentOutput, 0);
      }
    ...
    /*
     * if (gamePad0.getPOV() == 0 && upperSwitch.get()) { climb.set(.7); } else if
     * (gamePad0.getPOV() == 180 && lowerSwitch.get()) { climb.set(-.7); } else {//
     * default climb.set(0); }
     */
  ...
  public void limeDrive(double maxDriveStrength, double limeLightOffset) {
    if (v == 1) {
      ...
      double limeX = Math.signum(limRatX) * ((maxLimeX - minLimeX) / 2) * Math.sin(Math.PI * (limRatX - .5))
          + Math.signum(limRatX) * ((maxLimeX + minLimeX) / 2);
  ...
  public void autoIntake() {
    if (!gamePad0.getRawButton(6) || !gamePad0.getRawButton(5) || !(gamePad0.getPOV() == 270)) { // Not pressing these
                                                                                                 // buttons
